...
const RaceAnalysis = ({ runner, allRunners, onBack, onSelectRunner }) => {
  ...
  const stats = useMemo(() => {
    ...
    const sameCategory = allRunners.filter(
      (r) =>
        normalizeCategory(r.categoria) ===
          normalizeCategory(runner.categoria) && r.sexo === runner.sexo
    );
    ...
    const runnersAhead = sameCategory
      ...
      .map((r) => {
        ...
        const rTimeFormatted = `${rMinutes}:${rSeconds
          .toString()
          .padStart(2, "0")}`;
    ...
    const winnerPace = `${winnerPaceMinutes}:${winnerPaceSeconds
      .toString()
      .padStart(2, "0")}`;
    const winnerSpeed = (10 / (winnerTimeInSeconds / 3600)).toFixed(1);

    // Calculate percentile
    const generalPercentile = (
      (1 - runner.posicion / allRunners.length) *
      100
    ).toFixed(0);
    const categoryPercentile = (
      (1 - runner.posicionCategoria / categoryTotal) *
      100
    ).toFixed(0);
  ...
  if (!runner || !stats) {
    return (
      <div className="bg-gradient-to-br from-pink-500 via-purple-600 to-blue-900 p-6 min-h-screen flex items-center justify-center">
        ...
        <div className="bg-white rounded-xl shadow-2xl p-4 md:p-8">
          <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
            ...
            <div className="flex-1">
              <RunnerSearch runners={allRunners} onSelect={onSelectRunner} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="col-span-3 md:col-span-1 bg-gradient-to-r from-pink-500 to-pink-600 text-white p-6 rounded-lg shadow-lg text-center">
              <div className="text-4xl font-bold mb-2">{runner.posicion}°</div>
              <div className="text-sm uppercase tracking-wide">
                Posición General
              </div>
              <div className="text-xs mt-1 opacity-90">
                de {allRunners.length} corredores
              </div>
            </div>

            <div className="col-span-3 md:col-span-1 bg-gradient-to-r from-cyan-400 to-cyan-500 text-white p-6 rounded-lg shadow-lg text-center">
              <div className="text-4xl font-bold mb-2">
                {runner.posicionCategoria}°
              </div>
              <div className="text-sm uppercase tracking-wide">
                Categoría {normalizeCategory(runner.categoria)}
              </div>
              <div className="text-xs mt-1 opacity-90">
                de {stats.categoryTotal} competidores
              </div>
            </div>

            <div className="col-span-3 md:col-span-1 bg-gradient-to-r from-blue-800 to-blue-900 text-white p-6 rounded-lg shadow-lg text-center">
              <div className="text-4xl font-bold mb-2">
                {stats.timeFormatted}
              </div>
            ...
            <>
              <div className="mb-6">
                ...
                <p className="text-center text-gray-600 mb-4">
                  Cuánto tiempo te faltó para alcanzar cada posición (misma
                  categoría)
                </p>
              </div>
          ...
          <div className="mt-8 bg-cyan-50 border-l-4 border-cyan-500 p-6 rounded-r-lg">
            <h4 className="font-bold text-lg text-cyan-900 mb-3 flex items-center gap-2">
            ...
            <ul className="space-y-2 text-gray-700">
              {stats.runnersAhead.length > 0 && (
                <li>
                  ...
                  de alcanzar la posición {stats.runnersAhead[0].posicion} en tu
                  categoría
                </li>
              )}
              <li>
                • Ritmo promedio:{" "}
                <strong className="text-cyan-600">{stats.pace} min/km</strong> -
                Velocidad:{" "}
                <strong className="text-cyan-600">{stats.speed} km/h</strong>
              </li>
              <li>
                • El ganador de tu categoría corrió a{" "}
                <strong className="text-cyan-600">
                  {stats.winnerPace} min/km
                </strong>{" "}
                ({stats.winnerSpeed} km/h)
              </li>
              <li>
                • Posición en el{" "}
                <strong className="text-cyan-600">
                  top {stats.generalPercentile}%
                </strong>{" "}
                general y{" "}
                <strong className="text-cyan-600">
                  top {stats.categoryPercentile}%
                </strong>{" "}
                de tu categoría
              </li>
              <li>
                • Puntaje obtenido:{" "}
                <strong className="text-cyan-600">
                  {runner.puntaje} puntos
                </strong>
              </li>
            </ul>
          </div>

          <div className="mt-4 bg-gradient-to-r from-pink-50 to-pink-100 border-l-4 border-pink-500 p-6 rounded-r-lg">
            <div className="flex items-start gap-3">
              <Trophy className="h-6 w-6 text-pink-600 flex-shrink-0 mt-1" />
